...
public class ReservasView extends JFrame {
	...
	private long timeE, timeS, timeDifference;
	...
	public ReservasView() {
		...
		txtFechaE.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				try {
					timeE = txtFechaE.getCalendar().getTime().getTime();
					timeDifference = timeS - timeE;
					System.out.printf("timeE: %s, timeS: %s, timeDifference: %s\n", timeE, timeS, timeDifference);
					
					long days = TimeUnit.MILLISECONDS.toDays(timeDifference);
				    int priceForDay = 30000; // $ Pesos Colombianos	
					txtValor.setText(String.valueOf(priceForDay*days));
				}catch(NullPointerException nPointerException) {
					System.out.println(nPointerException.getMessage());
				}
			}
		});
		panel.add(txtFechaE);
		
		lblValorSimbolo = new JLabel("$ Pesos Colombianos");
		lblValorSimbolo.setBounds(173, 332, 184, 25);
		...
		txtFechaS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				try {
					timeS = txtFechaS.getCalendar().getTime().getTime();
					timeDifference = timeS - timeE;
					System.out.printf("timeE: %s, timeS: %s, timeDifference: %s\n", timeE, timeS, timeDifference);
				    
					long days = TimeUnit.MILLISECONDS.toDays(timeDifference);
				    int priceForDay = 30000;
					txtValor.setText(String.valueOf(priceForDay*days));
		...
		txtValor.setBounds(82, 328, 81, 33);
